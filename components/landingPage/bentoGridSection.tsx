  ...
  const cards = [
    {
      ...
      footer: (
        <div className="flex justify-between items-center w-full backdrop-blur-md bg-white/10 dark:bg-black/30 rounded-lg p-3 border border-white/20 dark:border-gray-800/30">
          <p className="text-emerald-700 dark:text-emerald-300 text-sm font-medium">Resultados visíveis em semanas</p>
            ...
            <Magnetic actionArea="global" intensity={0.2} range={200} springOptions={springOptions}>
              <div className="flex flex-col items-center justify-center">
                <p className="z-10 text-center text-4xl md:text-5xl font-bold tracking-tight group-hover:scale-105 transition-all duration-350 ease-in-out">
                  Transforme seu <span className="text-primary-500 dark:text-primary-400">corpo e vida</span>
                </p>
...

    {
      ...
      footer: (
        <div className="flex justify-between items-center w-full backdrop-blur-md bg-white/10 dark:bg-black/30 rounded-lg p-3 border border-white/20 dark:border-purple-900/30">
          <p className="text-purple-700 dark:text-purple-300 text-sm font-medium">Atualização contínua</p>
          <Button
            color="primary"
            variant="solid"
            radius="full"
    ...
    <section ref={sectionRef} className="container mx-auto px-4 py-16">
      ...
      >
        {cards.map((card, index) => (
          <motion.div
            key={index}
            className={cn(`relative h-[300px] col-span-12 block`, `md:col-span-${card.span || 4}`)}
          ...
          >
            ...
            >
              <AnimatePresence>
                {hoveredIndex === index && (
                  <motion.span
                    className={cn("absolute inset-0 h-full w-full bg-gradient-to-br rounded-3xl", card.color)}
              ...
              <Card className="relative z-10 w-full h-full overflow-hidden bg-white/5 dark:bg-black/5 backdrop-blur-sm border border-white/10 dark:border-gray-800/50 text-foreground shadow-md rounded-3xl">
                {card.content && <div className="absolute inset-0 z-0 pointer-events-none">{card.content}</div>}

                {card.title && card.titlePosition !== "bottom" && (
                  <div className="absolute z-10 top-4 left-4 flex items-start">
                    {card.icon && <div className="mr-3 p-2 rounded-full bg-white/10 backdrop-blur-sm">{card.icon}</div>}
                    <div className={card.title === "Dieta Personalizada" ? "pr-8" : ""}>
                      <h4 className={`${fontTitle.className} text-xl font-semibold`}>{card.title}</h4>
                      {card.description && (
                        <p
                ...
                {card.title && card.titlePosition === "bottom" && (
                  <div className="absolute z-10 bottom-4 left-4 flex items-start">
                    ...
                    <div>
                      <h4 className={`${fontTitle.className} text-xl font-semibold`}>{card.title}</h4>
                      {card.description && (
                        <p className={`${fontSubtitle.className} max-w-xs text-sm opacity-80`}>{card.description}</p>
                      )}
                    </div>
                  </div>
                )}

                {card.footer && <div className="absolute z-10 bottom-0 left-0 right-0 p-4">{card.footer}</div>}
              </Card>
            </NextLink>
          </motion.div>
        ))}
