  ...
  const projects = [
    // --- Row 1 ---
    {
      ...
      footer: (
        <>
          <div>
            <p className="text-secondary-400 dark:text-secondary-600 font-bold text-sm"> {/* Adjusted colors */}
              Resultados Visíveis
            </p>
            <p className="text-secondary-600 dark:text-secondary-900 text-xs"> {/* Adjusted colors */}
              O plano ideal para você.
            </p>
          </div>
          <Button className="text-tiny" color="secondary" radius="full"  variant="ghost">
            Ver Mais
          </Button>
        </>
      ),
    ...
    },
    {
      ...
      content: (
        <div className="absolute bottom-2 right-2 flex gap-1 opacity-40 z-0">
          <span className="text-xs p-1 bg-primary-600 text-secondary-100 rounded">IMC</span>
          <span className="text-xs p-1 bg-primary-600 text-secondary-100 rounded">TMB</span>
        </div>
      ),
    },
    {
          ...
          <div className="gap-2 flex h-full w-full flex-col items-center justify-center absolute z-10 p-4">
            <Magnetic actionArea="global" intensity={0.2} range={200} springOptions={springOptions}>
              <p className="z-10 whitespace-pre-wrap text-center text-2xl md:text-3xl font-medium tracking-tighter text-white group-hover:scale-105 transition-all duration-350 ease-in-out pb-1 md:pb-2">
                Transforme seu <span className="text-secondary-200">corpo</span>. {/* Adjusted color */}
              </p>
              <p className="group-hover:opacity-100 opacity-0 transition-all duration-350 ease-in-out text-center text-xs md:text-sm text-secondary-200"> {/* Adjusted color */}
          ...
          {projects.map((project, index) => (
            ...
            >
              ...
              <Card className="relative z-10 w-full h-full overflow-hidden bg-primary-500 text-secondary-100 transition-transform duration-300 ease-in-out group-hover:scale-[1.02]">
                ...
                {project.footer && (
                <CardFooter className="absolute bg-white/30 dark:bg-black/40 bottom-0 border-t border-secondary-300 dark:border-secondary-700 z-10 justify-between w-full p-4 rounded-b-lg"> {/* Adjusted footer style */}
                    {project.footer}
                </CardFooter>
                )}
              </Card>
