...
export default function ButtonForTest({ onOpen }: ButtonForTestProps) {
  return (
    ...
    <InteractiveHoverButton
      ...
      className="bg-moss_green text-secondary-200 font-semibold px-8 py-3 rounded-full hover:bg-dim_gray transition-colors cursor-pointer"
    >
      <span className="flex items-center justify-center relative z-10 gap-2">
       ...
       <span className={`${fontSubtitle.className} text-xs text-secondary-400 relative z-10 block mt-1`}>
          (IMC, TMB e % Gordura)
        </span>
    </InteractiveHoverButton>
  );
